...
//////////////////Comienzo de tratamiento de dependencia///////////////////////////
    ...
    jQuery("#producto, #color, #composicion, #calidad").on('change', function(e) {
    	...
    	//entradas
		if ((campo == 'calidad') && ( (hash_url=="/entradas") || (hash_url=="/editar_inventario") ) ) { //si calidad cambio de valor
    		if  ((val_calida != "0") && (val_calida != "") && (val_calida != null)) 
    		{
						...
						if(data != true){
							...
							jQuery('#foo').css('display','none');
								...
								jQuery.ajax({
									        ...
									        dataType : 'json',
									        success : function(data) {	
									        	MY_Socket.sendNewPost(data.vendedor+' - '+data.tienda);
	            ...
	            },
	            {
	                "render": function ( data, type, row ) {
						texto='<td><button'; 
							...
							texto+='<span  class="glyphicon glyphicon-add ">Agregar</span>';
						texto+='</button></td>';
						return texto;	
	                },
	                "targets": 8
...
////////////////////////////ordenar////////////////////////////////////////////////
	...
	jQuery('.buscar_prod_inven').on('typeahead:selected', function (e, datum,otro) {
	    ...
	    //jQuery('#color  option[value="val2').prop("selected", true);
	    //alert('asd');
	    //jQuery("#label_color").text(datum.id_color);
	    //jQuery("#color option[value='" + datum.id_color + "']").prop("selected",true);
	    //jQuery("#color option[value='" + datum.id_color + "']").eq(1).prop('selected', 'selected');
	    
	    
		jQuery('#producto option').each(function() {
		    if(jQuery(this).val() == datum.id_descripcion) {
		    	//alert(datum.id_descripcion);
		...
		console.log(jQuery('#color > option').val());

		jQuery('#color option').each(function() {
			alert('p');
		    /*
		    if(jQuery('#color option').val() == datum.id_color) {
		    	//alert(datum.id_descripcion);
		        jQuery('#color option').prop("selected", true);
		    }
		    */
		});		

		//jQuery('#producto').trigger("change");
		//jQuery('#color').val(datum.id_color);

		//console.log(jQuery("#color option[value='" + datum.id_color + "']").attr("selected"));
		/*jQuery('#color option').each(function() {
		    if(jQuery('#color').val() == datum.id_color) {
		    	//alert(datum.id_descripcion);
		        jQuery('#color').prop("selected", true);
		    }
		});	*/	

	  //  jQuery("#producto").eq(4).prop('selected', 'selected');
	    //jQuery('#color').val(datum.id_color);

	    /*
	    jQuery('#calidad').val(datum.id_color);
	    jQuery('#composicion').val(datum.id_descripcion);
	    */
	    
	    
	    //console.log(datum.id_color);
	    //jQuery('#color option:eq(1)').prop('selected', 'selected');
	    //jQuery('#color').val(datum.id_color);
	    //jQuery('#color option:eq('+datum.id_color+')').prop('selected', 'selected');


	   // jQuery('#codigo_original').val(datum.key);
	    
	    //provoca evento para que se actualice todo las listas
	    
	    
	    /*
	    jQuery('#producto').trigger( "change" );
	    jQuery('#color').val(datum.id_color);
	    jQuery('#composicion').val(datum.id_composicion);
	    jQuery('#calidad').val(datum.id_calidad);
		jQuery('#calidad option:eq(0)').prop('selected', 'selected');
		jQuery('#composicion option:eq(0)').prop('selected', 'selected');
		jQuery('#color option:eq(0)').prop('selected', 'selected');
		jQuery('#producto option:eq(0)').prop('selected', 'selected');
...
/*
jQuery('<button>Click para la suma de todos los (METROS) de las filas visibles</button>')
	.prependTo( 'section' )
	.on( 'click', function () {
		alert( 'Column sum is: '+ jQuery('#tabla_home').dataTable().api().column( 4, {page:'current'} ).data().sum() );
	} );
	*/


		var existencia = ['existencia', 'Descripción', 'Color', 'U.M', 'Kilogramos', 'Ancho', 'Proveedor', 'Lote', 'Ingreso'];
		var devolucion = ['devolucion', 'Descripción', 'Color', 'U.M', 'Kilogramos', 'Ancho', 'Proveedor', 'Lote', 'Ingreso'];
		var apartado = ['apartado', 'Descripción', 'Color', 'Pieza', 'U.M', 'Ancho', 'Dependencia', 'Tipo Apartado', 'Ingreso'];
    	var cero = ['Referencia', 'Nombre tela', 'Existencias', 'Imagen', 'Nombre Color', 'Color', 'Composición', 'Calidad', 'Precio'];
    	var baja = ['Referencia', 'Nombre tela', 'Existencias', 'Imagen', 'Nombre Color', 'Color', 'Composición', 'Calidad', 'Precio'];

    	var apartado_pendiente = ['Vendedor', 'Dependencia','Comprador', 'Fecha','Tipo Apartado','Detalles','Cancelar']; 
    	var arr_apartado_detalle = ['Código', 'Descripción', 'Color', 'Metros', 'Kilogramos', 'Ancho', 'Precio', 'Lote'];

    	var pedido_pendiente = ['Cliente', 'Dependencia','Consecutivo', 'Fecha','Tipo Apartado','Detalles','Cancelar']; 
    	var arr_pedido_detalle = ['Código', 'Descripción', 'Color', 'Metros', 'Kilogramos', 'Ancho', 'Precio', 'Lote'];
    	

//jQuery('#tabla_home').dataTable().fnPageChange( 'next' );
...
jQuery('#tabla_home').dataTable( {
	  ...
	  "pagingType": "full_numbers",
      "fnPreDrawCallback": function (oSettings) {
	      	//https://www.datatables.net/forums/discussion/13743/sinfo-reporting-wrong-start-value-when-idisplaylength-is-changed
	      	//http://stackoverflow.com/questions/24811581/datatable-how-to-set-page-start-correctly
...
"infoCallback": function( settings, start, end, max, total, pre ) {
    ...
    //var api = new jQuery.fn.dataTable( settings );
 
    // Output the data for the visible rows to the browser's console
    // You might do something more useful with it!
    //console.log( api);

    console.log(settings.json.totales);
    if (settings.json.totales) {
	    jQuery('#total_pieza').html( 'Total de piezas:'+ settings.json.totales.pieza);
	    jQuery('#total_kg').html( 'Total de Kgs:'+settings.json.totales.kilogramo);
		jQuery('#total_metro').html('Total de mtrs:'+ settings.json.totales.metro);
 ...
 "footerCallback": function( tfoot, data, start, end, display ) {
		...
		if  (data.length>0) {   //tiene q existir datos para mostrar Totales.
				total_metro = api
					.column( 9 )
					.data()
					.reduce( function (a, b) {
...
/*
    "columns": [
            ...
            { "title": "id_descripcion", "data": "id_descripcion" },
        ],
        */
...
jQuery('#tabla_apartado').dataTable( {
   ...
   "columnDefs": [
    			...
    			{ 
	                "render": function ( data, type, row ) {
    					 texto='<td><a href="apartado_detalle/'+jQuery.base64.encode(row[5])+'/'+jQuery.base64.encode(row[6])+'" '; 
						 	texto+=' class="btn btn-success btn-block">';
						 	texto+=' Detalles';
						 texto+='</a></td>';
    			...
    			{ 
	                "render": function ( data, type, row ) {
						...
						texto='<td><a href="eliminar_apartado_detalle/'+jQuery.base64.encode(row[5])+'/'+jQuery.base64.encode(row[6])+'" '; 
							texto+='class="btn btn-danger btn-sm btn-block" data-toggle="modal" data-target="#modalMessage">';
							texto+='<span class="glyphicon glyphicon-remove"></span>';
						texto+='</a></td>';
...
jQuery('#tabla_pedido').dataTable( {
   ...
   "columnDefs": [
    			...
    			{ 
	                "render": function ( data, type, row ) {
    					 texto='<td><a href="pedido_detalle/'+jQuery.base64.encode(row[5])+'" '; 
						 	texto+=' class="btn btn-success btn-block">';
						 	texto+=' Detalles';
						 texto+='</a></td>';
    			...
    			{ 
	                "render": function ( data, type, row ) {
						...
						texto='<td><a href="eliminar_pedido_detalle/'+jQuery.base64.encode(row[5])+'" '; 
							texto+='class="btn btn-danger btn-sm btn-block" data-toggle="modal" data-target="#modalMessage">';
							texto+='<span class="glyphicon glyphicon-remove"></span>';
						texto+='</a></td>';
					...
					jQuery.ajax({
						        ...
						        success : function(data) {	
						        	...
						        	jQuery.each(data.movimientos, function (i, valor) {
										...
										texto+='</td><td class="text-center">'+valor.cantidad_um+' '+valor.medida+'</td>';
										//texto+='<td class="text-center">'+valor.cantidad_um+'</td><td class="text-center">'+valor.ancho+' Mtrs</td>';
										texto+='</td><td class="text-center">'+valor.ancho+' Mtrs</td>';
										//texto+='<td class="text-center">'+valor.nombre+'</td><td class="text-center">'+valor.id_lote+'</td>';
										texto+='<td class="text-center">'+valor.nombre+'</td>';
										texto+='<td class="text-center">'+valor.id_lote+' - '+valor.consecutivo+'</td>';
										//texto+='<td class="text-center">'+valor.consecutivo+'</td>';
...
////////////////////////////ordenar////////////////////////////////////////////////
	...
	jQuery('.buscar_proveedor').on('typeahead:selected', function (e, datum,otro) {
	    ...
	    jQuery('#tabla_entrada').dataTable().fnDraw();
	    //console.log(key);
	    //jQuery('.buscar_proveedor.tt-input').attr("identificador",key);
	});	
					...
					{	
						...
						onChange: function(event, value, requiredValues, requirementsMet) {
							...
							jQuery.getJSON('catalogos/busqueda?elemento=completo', ajaxData, function(data) {
								...
								/*
									1.	Identificador del Proveedor ok
									2.	Referencia del Producto  ok
									3.	Identificador de lote   ok
									4.	Identificador de Fecha
									5.	Identificador de numero consecutivo OK
								...
								*/
								...
								//codigo=jQuery('.buscar_proveedor').typeahead('val')+'*'+data[0].referencia+'*'+'001'+'*'+fecha_formateada;
								codigo=codigo_proveedor+data[0].referencia+lote+fecha_formateada;
								
								//codigo=data[0].referencia;
								//console.log(codigo);
								//codigo="as";
								jQuery('#codigo').val(codigo);
								
								jQuery('#referencia').attr('value',data[0].referencia);

								//console.log(jQuery('.buscar_proveedor').typeahead('val'));	
